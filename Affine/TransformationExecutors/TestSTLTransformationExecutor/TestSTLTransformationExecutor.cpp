#include "TestSTLTransformationExecutor.h"
...
//void TestSTLTransformationExecutor::Multiply(std::vector<float>& vectorSet, std::pair<size_t, size_t> shape, float affineTransformation[9])
//{
...
//}
//
//void TestSTLTransformationExecutor::ReflectVectorSet(std::vector<float>& vectors, std::pair<size_t, size_t> shape, ReflectionPtr reflection)
//{
...
//}
//
//void TestSTLTransformationExecutor::Reflect(Result result, ReflectionPtr reflection)
//{
...
void TestSTLTransformationExecutor::ReflectVectorSet(std::vector<float>& vectors, std::pair<size_t, size_t> shape, ReflectionPtr reflection)
{
    auto Nrow = shape.first;
    auto Ncol = shape.second;

    auto& matr = vectors;
    std::array<float, 3> vect(reflection->normal);
    std::vector<float> scalar_ar(Ncol);
    float gamma, s;
    ...
    for (int column = 0; column < Ncol; column++)
    {
        for (int i = 0; i < Nrow; i++)
            scalar_ar[column] += matr[i * Ncol + column] * vect[i];
        scalar_ar[column] *= gamma;
    }
    for (int row = 0; row < Nrow; row++)
        for (int col = 0; col < Ncol; col++)
            matr[row * Ncol + col] -= scalar_ar[col] * vect[row];
}

Result TestSTLTransformationExecutor::Transform(const STLModelPtr model)
{
